...
public class Variable {
    ...
    public int offset;
    public DataType referenceType = DataType.Unknown;
    public DataType valueType = DataType.Unknown;

    public void registerUse(Command cmd, int index) {
        ParamInfo info = cmd.getParamInfo(index);
        if(info == null) return;

        if(valueType != referenceType && valueType != DataType.Unknown && info.absoluteType != valueType) {
